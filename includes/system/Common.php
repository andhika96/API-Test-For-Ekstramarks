<?php
	...
	if ( ! function_exists('display_content'))
	{
		...
		function display_content()
		{
			...
			{
				sys_notice('Failed', 'failed');
			}
			elseif (isset($_GET['status']) && $_GET['status'] == 'warning') 
			{
				sys_notice('Warning', 'warning');
			}
	...
	if ( ! function_exists('sys_notice')) 
	{
		...
		function sys_notice($notice, $type = 'failed')
		{
			if ($type == 'failed') 
			{
				$GLOBALS['notices'] = '<div class="ar-card card-full-color card-full-danger m-4" role="alert"><div class="card-body"><i class="fas fa-times-circle fa-lg fa-fw mr-1"></i> '.$notice.'</div></div>';
			}
			elseif ($type == 'success') 
			{
				$GLOBALS['notices'] = '<div class="ar-card card-full-color card-full-success m-4" role="alert"><div class="card-body"><i class="fas fa-check fa-lg fa-fw mr-1"></i> '.$notice.'</div></div>';
			}
			elseif ($type == 'warning') 
			{
				$GLOBALS['notices'] = '<div class="ar-card card-full-color card-full-warning m-4" role="alert"><div class="card-body"><i class="fas fa-exclamation-triangle fa-lg fa-fw mr-1"></i> '.$notice.'</div></div>';
			}			
		}
	}
