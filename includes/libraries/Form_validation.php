<?php
	 ...
	 *	Information file: Form Validation Class
	 */

defined('BASEPATH') OR exit('No direct script access allowed');

class ARUNA_Form_validation {
	...
	/**
	 * The loaded instances of our config validation files.
	 *
	 * @var array
	 */
	...
	public function __construct($rules = array())
	{
		...
		{
			$set->checkFullname('Gumara');
			$out = $set->setCustomError();
		}
	...
	public function set_rules($field, $label = '', $rules = array(), $errors = array())
	{
		...
		if ($this->input->method() !== 'post' && empty($this->validation_data))
		{
			return $this;
		}
	...
	public function run($group = '')
	{
		...
		if (count($this->_field_data) === 0)
		{
			...
			if (empty($group))
			{
				...
				$group = $this->uri->uri_string();
				isset($this->_config_rules[$group]) OR $group = $this->uri->uri_string();
			}
	...
	protected function _execute($row, $rules, $postdata = NULL, $cycles = 0)
	{
		...
		foreach ($rules as $rule)
		{
				...
				if ($callback)
				{
					foreach ($this->ruleSetInstances as $set)
					{
						...
						else
						{
							...
							$result = $set->$rule($postdata, $param);
						}
					}
				}
				else
			...
			if ($result === FALSE)
			{
				...
				if ( ! is_string($rule))
				{
					$line = $lang->line('form_validation_error_message_not_set').' (Anonymous function)';
				}
				else
				{
	...
	protected function _get_error_message($rule, $field)
	{
		...
		elseif ($set->setCustomError($rule))
		{
			return $set->setCustomError($rule);
		}
		elseif (FALSE !== ($line = $lang->line('form_validation_'.$rule)))
		{
			return $line;
		}
		// DEPRECATED support for non-prefixed keys, lang file again
		elseif (FALSE !== ($line = $lang->line($rule, FALSE)))
		{
			return $line;
		}
		
		return $lang->line('form_validation_error_message_not_set').' ('.$rule.')';
	}
	...
	protected function _translate_fieldname($fieldname)
	{
		...
		if (sscanf($fieldname, 'lang:%s', $line) === 1 && FALSE === ($fieldname = $lang->line($line, FALSE)))
		{
			// Disabled by Andhika Adhitia N
			// return $line;
			show_error('Translation field name is not support');
		}
	 ...
	 * ARUNA\Config\Validation and stores them locally so we can use them.
	 */

	protected function loadRuleSets()
	{
		if ( ! file_exists($file_path = BASEPATH.'config/validation.php')) 
		{
			show_error('Missing file config validation.');
		}

		include BASEPATH.'config/validation.php';

		$this->ruleSetInstances[] = new Aruna\Config\validation;
	}
	...
	public function set_value($field = '', $default = '', $html_escape = TRUE)
	{
		...
		return ($html_escape) ? html_escape($this->_field_data[$field]['postdata']) : $this->_field_data[$field]['postdata'];
	}
	...
	public function is_unique($str, $field)
	{
		...
		sscanf($field, '%[^.].%[^.].%[^.]', $selectdb, $table, $field);

		$conn 		= load_db($selectdb, $db[$selectdb]['dbtype']);
		$res  		= $conn->sql_prepare("select count(*) as num from $table where $field = :$field limit 1");
		$bindParam 	= $conn->sql_bindParam([$field => $str], $res);
		$row 		= $conn->sql_fetch_array($bindParam); 
	...
	{
		return $this->security->strip_image_tags($str);
	}
